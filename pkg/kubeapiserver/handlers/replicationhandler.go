...
// change the ReplicationResourceVersion
...
// updateReplica the helper function to update the replica information
func updateReplica(replica *apiobject.ReplicationController, key string) error {
	replica.Data.ResourcesVersion = apiobject.UPDATE
	err := replicaStorageTool.GuaranteedUpdate(context.Background(), key, replica)
	if err != nil {
		log.Error("[UpdateReplicaHandler] update replica information error, ", err)
		return err
	}
	return nil
}

// CreateReplicaHandler the url format is /api/v1/namespaces/{namespace}/replicas
func CreateReplicaHandler(c *gin.Context) {
	...
	var prevReplica apiobject.ReplicationController
...
// UpdateReplicaHandler the url format is POST /api/v1/namespaces/:namespace/replicas/:name/update
func UpdateReplicaHandler(c *gin.Context) {
	...
	if err := updateReplica(replica, key); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
